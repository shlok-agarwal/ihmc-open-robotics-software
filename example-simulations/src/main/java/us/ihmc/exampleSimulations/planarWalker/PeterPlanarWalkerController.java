...
public class PeterPlanarWalkerController implements RobotController
{
   ...
   private StateMachine<ControllerState, FinishableState> stateMachine;
   ...
   private StateMachine<ControllerState, FinishableState> initializeStateMachine()
   {
      FinishableStateMachineFactory<ControllerState, FinishableState> factory = new FinishableStateMachineFactory<>(ControllerState.class);
      factory.setNamePrefix("controllerState").setRegistry(registry).buildYoClock(robot.getYoTime());
      factory.addFinishableState(ControllerState.START, new StartState(), ControllerState.RIGHT_SUPPORT);
      factory.addFinishableState(ControllerState.RIGHT_SUPPORT, new SingleSupportState(RobotSide.RIGHT), ControllerState.LEFT_SUPPORT);
      factory.addFinishableState(ControllerState.LEFT_SUPPORT, new SingleSupportState(RobotSide.LEFT), ControllerState.RIGHT_SUPPORT);

      return factory.build(ControllerState.START);
   }

   private class StartState implements FinishableState
   {
      @Override
   ...
   private class SingleSupportState implements FinishableState
   {
