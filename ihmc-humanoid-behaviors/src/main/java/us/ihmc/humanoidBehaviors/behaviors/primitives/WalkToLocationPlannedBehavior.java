...
public class WalkToLocationPlannedBehavior extends StateMachineBehavior<WalkToLocationStates>
{
   ...
   @Override
   protected WalkToLocationStates configureStateMachineAndReturnInitialKey(FinishableStateMachineFactory<WalkToLocationStates, BehaviorAction> factory)
   {
      ...
      factory.addFinishableState(WalkToLocationStates.SETUP, setUpState, WalkToLocationStates.PLAN_PATH);
      factory.addState(WalkToLocationStates.PLAN_PATH, planFootSteps);
      factory.addFinishableState(WalkToLocationStates.WALK_PATH, sendPlanToController, WalkToLocationStates.DONE);
      factory.addFinishableState(WalkToLocationStates.PLAN_FAILED, planFailedState, WalkToLocationStates.DONE);
      factory.addState(WalkToLocationStates.DONE, doneState);

      factory.addTransition(WalkToLocationStates.PLAN_PATH, time -> {
         if (!isPlanPathComplete())
            return null;
         return hasValidPlanPath() ? WalkToLocationStates.WALK_PATH : WalkToLocationStates.PLAN_FAILED;
      });
