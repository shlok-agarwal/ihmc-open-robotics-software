...
//      communicationBridge.registerYovaribleForAutoSendToUI(statemachine.getStateYoVariable()); // FIXME
   ...
   @Override
   protected TurnValveBehaviorState configureStateMachineAndReturnInitialKey(FinishableStateMachineFactory<TurnValveBehaviorState, BehaviorAction> factory)
   {
      ...
      factory.addFinishableState(TurnValveBehaviorState.SETUP_ROBOT, setup, TurnValveBehaviorState.SEARCHING_FOR_VALVE);
      factory.addFinishableState(TurnValveBehaviorState.SEARCHING_FOR_VALVE, searchForValveFar, TurnValveBehaviorState.WALKING_TO_VALVE);
      factory.addFinishableState(TurnValveBehaviorState.WALKING_TO_VALVE, walkToValveAction, TurnValveBehaviorState.SEARCHING_FOR_VALVE_FINAL);
      factory.addFinishableState(TurnValveBehaviorState.SEARCHING_FOR_VALVE_FINAL, searchForValveNear, TurnValveBehaviorState.TURNING_VALVE);
      factory.addFinishableState(TurnValveBehaviorState.TURNING_VALVE, graspAndTurnValve, TurnValveBehaviorState.WAITING_FOR_USER_CONFIRMATION);

      factory.addState(TurnValveBehaviorState.WAITING_FOR_USER_CONFIRMATION, getUserValidation);
      factory.addTransition(TurnValveBehaviorState.WAITING_FOR_USER_CONFIRMATION, timeInCurrentState -> {
         if (!isValidationDone())
            return null;
         return hasUserValidated() ? TurnValveBehaviorState.BACK_AWAY_FROM_VALVE : TurnValveBehaviorState.TURNING_VALVE;
      });
