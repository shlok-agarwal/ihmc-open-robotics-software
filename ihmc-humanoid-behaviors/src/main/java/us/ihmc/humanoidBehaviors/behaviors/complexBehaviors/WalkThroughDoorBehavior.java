...
public class WalkThroughDoorBehavior extends StateMachineBehavior<WalkThroughDoorBehaviorState>
{
   ...
   @Override
   protected WalkThroughDoorBehaviorState configureStateMachineAndReturnInitialKey(FinishableStateMachineFactory<WalkThroughDoorBehaviorState, BehaviorAction> factory)
   {
      ...
      factory.addFinishableState(WalkThroughDoorBehaviorState.SETUP_ROBOT, setup, WalkThroughDoorBehaviorState.SEARCHING_FOR_DOOR);
      factory.addFinishableState(WalkThroughDoorBehaviorState.SEARCHING_FOR_DOOR, searchForDoorFar, WalkThroughDoorBehaviorState.WALKING_TO_DOOR);

      factory.addState(WalkThroughDoorBehaviorState.WALKING_TO_DOOR, walkToDoorAction);
      factory.addTransition(WalkThroughDoorBehaviorState.WALKING_TO_DOOR, timeInCurrentState -> {
         if (!isWalkingDone())
            return null;
         return hasWalkingSucceded() ? WalkThroughDoorBehaviorState.SEARCHING_FOR_DOOR_FINAL : WalkThroughDoorBehaviorState.FAILED;
      });

      factory.addFinishableState(WalkThroughDoorBehaviorState.SEARCHING_FOR_DOOR_FINAL, searchForDoorNear,
                                 setUpArms ? WalkThroughDoorBehaviorState.SET_UP_ROBOT_FOR_DOOR_WALK : WalkThroughDoorBehaviorState.WALK_THROUGH_DOOR);
      factory.addFinishableState(WalkThroughDoorBehaviorState.SET_UP_ROBOT_FOR_DOOR_WALK, setUpForWalk, WalkThroughDoorBehaviorState.WALK_THROUGH_DOOR);
      factory.addFinishableState(WalkThroughDoorBehaviorState.WALK_THROUGH_DOOR, walkThroughDoor, WalkThroughDoorBehaviorState.RESET_ROBOT);
      factory.addFinishableState(WalkThroughDoorBehaviorState.RESET_ROBOT, resetRobot, WalkThroughDoorBehaviorState.DONE);
      factory.addFinishableState(WalkThroughDoorBehaviorState.FAILED, failedState, WalkThroughDoorBehaviorState.DONE);
